...
while True:
    ...
    # แสดงป้ายล่าสุด
    cv2.putText(display_frame, plate_text, (10,50), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0,255,0), 3)
    cv2.imshow("License Plate OCR", display_frame)

    # ส่ง API ทุก 1 วินาที
    if time.monotonic() - last_time > 1:
        ...
        try:
            response = requests.post(url, files=files, data=payload, headers=headers, timeout=5)

            text = response.text.strip()
            if text:
                try:
                    # บาง API ใช้ single quote ' แทน double quote "
